...
SELECT 
    ...
    pd.age,
    pd.gender,
    sc.current_home_county,
    sc.secondary_school_county,
    sc.residence,
    sc.stud_email,
    cd.next_of_kin_name,
    ...
    fd.fee_paid
FROM personal_details pd
JOIN school_details sc ON pd.stud_ID = sc.stud_ID
JOIN contact_details cd ON sc.stud_email = cd.stud_email
...
SET fd.stud_name = (
	SELECT pd.stud_name
    FROM personal_details pd
    WHERE pd.stud_name = fd.stud_name
)
...
CREATE VIEW financial_detail_view AS
...
SELECT sd.secondary_school_county, SUM(CASE WHEN pd.gender = 'Male' THEN 1 ELSE 0 END) AS male_count, SUM(CASE WHEN pd.gender = 'Female' THEN 1 ELSE 0 END) AS female_count
FROM personal_details pd 
JOIN school_details sd ON pd.stud_ID = sd.stud_ID
...
SELECT next_of_kin_relation, ROUND((COUNT(next_of_kin_relation) * 100 / (SELECT COUNT(*) FROM contact_details)), 2) AS percentage
